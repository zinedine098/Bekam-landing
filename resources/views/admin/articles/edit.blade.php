        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-primary/5 overflow-hidden">
            <div class="p-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Perbarui konten artikel</p>
                </div>
            ...
            <form action="{{ route('articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        @if($article->image)
                            <div class="col-span-3">
                                <div class="relative rounded-xl overflow-hidden aspect-video border border-slate-200 dark:border-slate-800">
                                    <img src="{{ Storage::url($article->image) }}" class="w-full h-full object-cover">
                                </div>
                                <p class="text-[10px] text-center mt-1 text-slate-400">Gambar Saat Ini</p>
                            </div>
                        @endif
                        <div class="{{ $article->image ? 'col-span-9' : 'col-span-12' }}">

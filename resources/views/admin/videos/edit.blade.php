        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-primary/5 overflow-hidden">
            <div class="p-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Perbarui informasi video</p>
                </div>
            ...
            <form action="{{ route('videos.update', $video) }}" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        ...
                        placeholder="Deskripsi singkat tentang video...">{{ old('description', $video->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
